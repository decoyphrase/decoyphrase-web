...
const CG_IDS = {
  ...
  BTC: 'bitcoin',
  SUI: 'sui',
  POLYGON: 'matic-network', // Polygon uses MATIC/POL token
};
...
async function getPrices() {
  try {
    ...
    const res = await fetch(
      ...
      {
        next: { revalidate: 300 }, // Share cache with other requests
        headers: {
    ...
    return {}; // Return empty object to not crash, calculation will result in 0 USD
  }
}
